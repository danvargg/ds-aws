...
df_transformed = df.rename(
    columns={'Review Text': 'review_body', 'Rating': 'star_rating', 'Class Name': 'product_category'}
)
df_transformed.drop(
    columns=[
        'Clothing ID', 'Age', 'Title', 'Recommended IND', 'Positive Feedback Count', 'Division Name', 'Department Name'
    ],
    inplace=True
)
...
df_transformed = df_transformed.groupby('product_category').filter(
    lambda reviews: len(reviews) > 10
...
# Visualization
def show_values_barplot(axs: np.ndarray, space) -> None:
    """
    Show values on top of barplot.

    Args:
        axs: Matplotlib axis object.
        space: Space between bar and value.
    """
